...
<body>
	...
	// pagina de maquinas no patio com limite
	...
	if($_SESSION['busca'] == 1){
		if($areaUsuario == 'Manutenção'){
			$resultado = $mysqli->query("SELECT * FROM patiomaquinas WHERE status LIKE '%".$_SESSION['status']."%' AND nomeMaquina LIKE '%".$_SESSION['modelo']."%' AND nomeMaquina LIKE '%".$_SESSION['fabricante']."%' AND nomeMaquina LIKE '%".$_SESSION['patrimonio']."%' AND idEmpresa = '$idEmpresa' ORDER BY disponivel ASC, idMaquina ASC LIMIT 0,5");
		}else{
			$resultado = $mysqli->query("SELECT * FROM patiomaquinas WHERE status LIKE '%".$_SESSION['status']."%' AND nomeMaquina LIKE '%".$_SESSION['modelo']."%' AND nomeMaquina LIKE '%".$_SESSION['fabricante']."%' AND nomeMaquina LIKE '%".$_SESSION['patrimonio']."%' AND idEmpresa = '$idEmpresa' ORDER BY disponivel DESC, idMaquina ASC LIMIT 0,5");
		}
	}else{
		if($areaUsuario == 'Manutenção'){
			$resultado = $mysqli->query("SELECT * FROM patiomaquinas WHERE idEmpresa = '$idEmpresa' ORDER BY disponivel ASC, idMaquina ASC LIMIT 0,5");
		}else{
			$resultado = $mysqli->query("SELECT * FROM patiomaquinas WHERE idEmpresa = '$idEmpresa' ORDER BY disponivel DESC, idMaquina ASC LIMIT 0,5");
		}
	}
	if($_SESSION['busca'] == 1){
	...
	if($verifica == 0){ ?>
		...
		while($reg_cadastro= mysqli_fetch_array($resultado)){
			...
			$dtRetornoConvert = strtotime($dtRetorno);
			// verifica a diferença em segundos entre as duas datas e divide pelo número de segundos que um dia possui
			if($dtRetorno != "0000-00-00"){
				$contadorRetorno = ($dataAtualConvert - $dtRetornoConvert) /86400;
			}

			// converte as datas para o formato timestamp
			
			$dtDisponivelConvert = strtotime($dtDisponivel);
			// verifica a diferença em segundos entre as duas datas e divide pelo número de segundos que um dia possui
			if($dtDisponivelConvert != "0000-00-00"){
				$contadorDisponivel = ($dataAtualConvert - $dtDisponivelConvert) /86400;
			}
			...
			<div id="frases" class="box-info-patio-maq">
				<?php if($disponivel == "1"){?>
					...
					if($TotalPatio  > '5'){
						?>
						<div id="limitPM"><button id="limitPM" type="submit" onclick="limitPM();" class="btn-carrega">Carregar todas as maquinas</button></div>
					<?php } ?>
				</body>
				</html>
