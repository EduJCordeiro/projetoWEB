...
// pagina de saida de maquinas sem limite
...
if($_SESSION['busca'] == 1){
	$resultado =  $mysqli->query("SELECT * FROM saidamaquina WHERE status LIKE '%".$_SESSION['status']."%' AND nomeMaquina LIKE '%".$_SESSION['modelo']."%' AND nomeMaquina LIKE '%".$_SESSION['fabricante']."%' AND nomeMaquina LIKE '%".$_SESSION['patrimonio']."%' AND idEmpresa = '$idEmpresa' ORDER BY dataSaida, horaSaida ASC");
}else{
	$resultado = $mysqli->query("SELECT * FROM saidamaquina WHERE idEmpresa = '$idEmpresa' ORDER BY dataSaida, horaSaida ASC");
}
