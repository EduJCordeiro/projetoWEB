...
// pagina de maquinas retornando com limite
...
if($_SESSION['busca'] == 1){
	$resultadoPM = $mysqli->query("SELECT * FROM retornomaquinas WHERE nomeMaquina LIKE '%".$_SESSION['modelo']."%' AND nomeMaquina LIKE '%".$_SESSION['fabricante']."%' AND nomeMaquina LIKE '%".$_SESSION['patrimonio']."%' AND idEmpresa = '$idEmpresa' ORDER BY dataRetorno, horaRetorno ASC LIMIT 0,5");
}else{
	$resultadoPM = $mysqli->query("SELECT * FROM retornomaquinas WHERE idEmpresa = '$idEmpresa' ORDER BY dataRetorno, horaRetorno ASC LIMIT 0,5");
}
...
<div class="box-main">
	...
	if($verifica == 0){ ?>
		...
		if($TotalRetorno  > '5'){
	?>
<div id="limitMR"><button id="limitMR" type="submit" onclick="limitMR();" class="btn-carrega">Carregar todas as maquinas</button></div>
<?php } ?>
</head>
</html>
