...
<body>
<?php
// pagina de maquinas locadas com limite
...
if($_SESSION['busca'] == 1){
	$resultadoML = $mysqli->query("SELECT * FROM maquinas WHERE fase LIKE '%".$_SESSION['status']."%' AND modelo LIKE '%".$_SESSION['modelo']."%' AND fabricante LIKE '%".$_SESSION['fabricante']."%' AND patrimonio LIKE '%".$_SESSION['patrimonio']."%'
		 AND idEmpresa = '$idEmpresa' AND fase = '3' AND excluido = '0' ORDER BY futuro DESC, idMaquina ASC LIMIT 0,5");
}else{
	$resultadoML = $mysqli->query("SELECT * FROM maquinas WHERE idEmpresa = '$idEmpresa' AND fase = '3' AND excluido = '0' ORDER BY futuro DESC, idMaquina ASC LIMIT 0,5");
}
...
<div class="box-main">
	...
	if($verifica == 0){ ?>
		...
		while($reg_cadastro= mysqli_fetch_array($resultadoML)){
			...
			$dataAtualConverter = strtotime($dataAtual);
			$dtLocacaoConvert = strtotime($dtLocacao);
			// verifica a diferença em segundos entre as duas datas e divide pelo número de segundos que um dia possui
			if($dtLocacaoConvert != "0000-00-00"){
				$contadorLocacao = ($dataAtualConverter - $dtLocacaoConvert) /86400;
			}
			// ==========================================================================================================================================
			?>
		...
		if($TotalLocada  > '5'){
		?>
<div id="limitML"><button id="limitML" type="submit" onclick="limitML();" class="btn-carrega">Carregar todas as maquinas</button></div>
<?php } ?>
</body>
</html>
